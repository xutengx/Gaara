...
trait WechatTrait {
	...
	public function getInfoOnWechatProfessional($is = false) {
		...
		if (strpos($user_agent, 'MicroMessenger') === false) {
			...
			$code	 = obj('F')->get('code');
			//获取授权
			$auth	 = obj('\Gaara\Expand\Wechat', true, $appid, $appsecret);
			if ($code === null) {
				...
				if ($explicit)
					$this->wechatinfo	 = $auth->get_user_info($res['access_token'], $res['openid']);
				else
					$this->wechatinfo	 = array('openid' => $res['openid']);
				$this->main_getInfo();
			}
		}
	}
