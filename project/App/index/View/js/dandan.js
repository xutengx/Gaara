...
//创建新用户
    function newuser($this, arr, i, ing) {
        ...
        if (ing != undefined) {//创建最近聊天
            //alert("最近聊天为真");
            $($this).prepend('<li id="' + id + i + '" user_id="'+arr[2] +'">' + arr[0] + '</li>');
            $('#' + id + i).click(function () {
                title_newuser('title_' + id + ing, arr[0], arr[1],arr[2]);
            });//给按钮加事件
        } else {//创建好友
            $($this).append('<li id="' + id + i + '"user_id="'+arr[2] +'">' + arr[0] + '</li>');
            $('#' + id + i).click(function () {
                title_newuser('title_' + id + i, arr[0], arr[1],arr[2]);
            });//给按钮加事件
        }
...
//创建标题栏和主控制（原是有一个主控制，忘了，就合在一起了，哈哈）
    function title_newuser(id, user, img, uid) {
        if ($("#" + id).length < 1) {
            $("#mid_top").append('<div id="' + id + '" class="list" user_id="'+uid+'"><table border="0" cellspacing="0" cellpadding="0"><tr><td id="zi' + id + '" class="td_user td_user_click">' + user + '</td><td id="zino' + id + '" class="td_hide td_hide_click">X</td></tr></table></div>');
            //创建完成后给事件
            //alert('#'+id)
            $('#' + id).click(function () {
...
//发送后调用此方法
    function saysay() {
        ...
        if ($("#texterea").val()) {
            ...
            var say = {};
            say.to_id = $(".list").attr('user_id');
            say.content = $("#right_mid").html();
            say.type = 0;
            ws.send(JSON.stringify(say));
            //console.log($(".list").attr('user_id'));
            //console.log($("#right_mid").html());
