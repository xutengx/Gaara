...
class obj{
    ...
    public static function requireClass($class){
        if(isset(self::$obj_map[$class])) require(ROOT.self::$obj_map[$class]);
        else {
            if( ($is = strrpos($class, '\\')) !== false) {
                $app = substr($class, 0 ,(int)$is);
                $classname = substr($class, (int)$is+1);
            }else $classname = $class;
            if (strtolower(substr($class, -5)) == 'contr') require(ROOT.'Application/'.$app.'/Controller/'.$classname.'.class.php');
            else if (strtolower(substr($class, -6)) == 'module') require(ROOT.'Application/'.$app.'/Module/'.$classname.'.class.php');
            else if (strtolower(substr($class, -4))=='tool') require(ROOT . 'tool/'. $classname . '.class.php');
            else  {
                ...
                if(file_exists($str0)) require $str0;
                else if(file_exists($str)) require $str;
                else require(ROOT . 'Include/' . $classname . '.class.php');
            }
        }
    }
    // 判断 $class 类型
